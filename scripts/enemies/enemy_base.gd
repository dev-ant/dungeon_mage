...
const HIT_FLASH_NEUTRAL_COLOR := Color(2.0, 2.0, 2.0, 1.0)
const HIT_FLASH_ACTIVE_COLOR := Color(2.14, 1.96, 2.28, 1.0)
const HIT_FLASH_AFTERSHOCK_COLOR := Color(2.18, 2.04, 1.84, 1.0)
...
func _sync_health_bar_visibility() -> void:
	var visible := should_show_world_health_bar()
	if health_bar_bg != null:
		health_bar_bg.visible = visible
	if health_bar_fill != null:
		health_bar_fill.visible = visible


func should_show_world_health_bar() -> bool:
	return enemy_grade != "boss"


func should_show_target_health_bar() -> bool:
	return enemy_grade == "boss"
...
func _physics_process(delta: float) -> void:
	...
	if enemy_sprite != null:
		enemy_sprite.modulate = _get_hit_flash_color() if hit_flash_timer > 0.0 else visual_modulate
		EnemyVisualLibrary.update_animation(
			enemy_sprite, behavior_state, enemy_type, mushroom_stun_attack_active, velocity
		)
	else:
		body_polygon.color = _get_hit_flash_color() if hit_flash_timer > 0.0 else tint
...
func is_room_transition_blocking() -> bool:
	return not is_dying
...
func _get_hit_flash_color() -> Color:
	if GameState.soul_dominion_active:
		return HIT_FLASH_ACTIVE_COLOR
	if GameState.soul_dominion_aftershock_timer > 0.0:
		return HIT_FLASH_AFTERSHOCK_COLOR
	return HIT_FLASH_NEUTRAL_COLOR
