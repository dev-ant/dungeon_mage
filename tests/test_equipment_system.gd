...
func _equipment_display_name(item_id: String) -> String:
	return str(GameDatabase.get_equipment(item_id).get("display_name", item_id))


func _equipment_preset_summary_fragment(preset_id: String) -> String:
	return "프리셋:%s" % GameState.get_equipment_preset_label(preset_id)
...
func _wait_for_payload_multi_hit_resolution(payload: Dictionary, extra_buffer: float = 0.08) -> void:
	var hit_count := maxi(1, int(payload.get("multi_hit_count", 1)))
	var wait_time := maxf(float(payload.get("hit_interval", 0.0)), 0.0) * float(hit_count - 1) + extra_buffer
...
func test_equipment_preset_changes_runtime_damage_and_summary() -> void:
	...
	assert_string_contains(
		...
		_equipment_preset_summary_fragment("fire_burst")
	)
...
func test_fire_burst_preset_increases_fire_bolt_damage_against_brute() -> void:
	...
	await _wait_for_payload_multi_hit_resolution(base_payload)
	...
	await _advance_frames(1)
	projectile._hit_enemy(enemy)
	await _wait_for_payload_multi_hit_resolution(boosted_payload)
...
func test_wind_tempo_preset_improves_gale_cutter_operation_against_leaper() -> void:
	...
	for raw_payload in payloads:
		var hit_payload := (raw_payload as Dictionary).duplicate(true)
		projectile = _spawn_projectile_for_equipment_coverage(root, hit_payload)
		await _advance_frames(1)
		projectile._hit_enemy(enemy)
		await _wait_for_payload_multi_hit_resolution(hit_payload)
		if is_instance_valid(projectile):
			projectile.queue_free()
	var base_damage: int = hp_before - enemy.health
	var base_speed: float = Vector2(base_payload.get("velocity", Vector2.ZERO)).length()
	var base_emitted_hits := payloads.size()
	...
	assert_gt(
		payloads.size(),
		base_emitted_hits,
		"wind_tempo preset must improve Gale Cutter operation with extra projectile emits"
	)
	var boosted_payload: Dictionary = payloads[0].duplicate(true)
	for raw_payload in payloads:
		var hit_payload := (raw_payload as Dictionary).duplicate(true)
...
func test_can_change_individual_equipment_slot() -> void:
	...
	assert_string_contains(GameState.get_equipment_summary(), _equipment_display_name("weapon_tempest_rod"))
...
func test_can_grant_equipment_into_inventory() -> void:
	...
	assert_string_contains(
		GameState.get_equipment_inventory_summary(),
		_equipment_display_name("weapon_tempest_rod")
	)
...
func test_grant_equipment_uses_first_empty_slot_after_manual_reposition() -> void:
	assert_true(GameState.grant_equipment_item("weapon_tempest_rod"))
	assert_true(GameState.move_equipment_inventory_item(0, 5))
	assert_true(GameState.grant_equipment_item("focus_storm_orb"))
	assert_eq(GameState.get_equipment_inventory_item_at(0), "focus_storm_orb")
	assert_eq(GameState.get_equipment_inventory_item_at(5), "weapon_tempest_rod")
...
func test_slot_inventory_summary_filters_by_slot() -> void:
	...
	assert_true(GameState.grant_equipment_item("focus_storm_orb"))
	assert_string_contains(
		GameState.get_equipment_slot_inventory_summary("weapon"),
		_equipment_display_name("weapon_tempest_rod")
	)
	assert_false(
		GameState.get_equipment_slot_inventory_summary("weapon")
			.contains(_equipment_display_name("focus_storm_orb"))
	)
