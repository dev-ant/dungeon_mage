...
func test_normal_enemy_uses_world_health_bar_and_not_target_panel() -> void:
	var enemy = autofree(ENEMY_SCRIPT.new())
	enemy.configure({"type": "brute", "position": Vector2.ZERO}, null)
	assert_true(enemy.should_show_world_health_bar())
	assert_false(enemy.should_show_target_health_bar())


func test_boss_enemy_uses_target_panel_and_hides_world_health_bar() -> void:
	var enemy = autofree(ENEMY_SCRIPT.new())
	enemy.configure({"type": "boss", "position": Vector2.ZERO}, null)
	assert_false(enemy.should_show_world_health_bar())
	assert_true(enemy.should_show_target_health_bar())
...
func test_soul_dominion_active_pushes_enemy_hit_flash_toward_cooler_violet() -> void:
	GameState.reset_progress_for_tests()
	var enemy = autofree(ENEMY_SCRIPT.new())
	enemy.configure({"type": "brute", "position": Vector2.ZERO}, null)
	var safe_flash: Color = enemy._get_hit_flash_color()
	GameState.soul_dominion_active = true
	var active_flash: Color = enemy._get_hit_flash_color()
	assert_gt(
		active_flash.b, safe_flash.b, "Active Soul Dominion hit flash must lean cooler than the neutral white flash"
	)
	assert_lt(
		active_flash.g,
		safe_flash.g,
		"Active Soul Dominion hit flash must trim green so the enemy read skews slightly more violet"
	)


func test_soul_dominion_aftershock_keeps_enemy_hit_flash_warmer_than_active_state() -> void:
	GameState.reset_progress_for_tests()
	var enemy = autofree(ENEMY_SCRIPT.new())
	enemy.configure({"type": "brute", "position": Vector2.ZERO}, null)
	GameState.soul_dominion_active = true
	...
	var aftershock_flash: Color = enemy._get_hit_flash_color()
	assert_gt(
		aftershock_flash.r,
		active_flash.r,
		"Aftershock hit flash must recover some warmth so it reads softer than the fully active risk state"
	)
	assert_lt(
		aftershock_flash.b,
		active_flash.b,
		"Aftershock hit flash must lose some of the active state's cool violet pressure"
	)
