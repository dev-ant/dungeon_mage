...
2026-04-03 기준 `buff_combos.json`에서 실제 관측된 `effect_tags`는 아래 15개입니다.

| 태그 | 의미 | 현재 예시 |
| --- | --- | --- |
| `aftercast_cut` | 후딜 감소 | Overclock Circuit |
| `ash_stack` | 잿재 중첩 축적 | Ashen Rite |
| `cast_speed_up` | 시전 속도 증가 | Time Collapse |
| `chain_clear` | 연쇄 정리/연쇄 폭발 | Funeral Bloom |
| `chain_up` | 체인 수 증가 | Overclock Circuit |
| `cheap_first_casts` | 첫 연속 시전 자원 할인 | Time Collapse |
| `dash_cast` | 대시 직후 시전 보정 | Overclock Circuit |
