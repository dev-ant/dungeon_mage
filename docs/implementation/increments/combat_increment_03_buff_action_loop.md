...
- `Main` 런타임이 combo burst projectile의 최소 지속시간을 `0.35s`로 잠가 headless 프레임 편차에서도 event camera 포커스와 projectile spawn 검증이 흔들리지 않도록 정리
- `get_combo_summary()`에서 `[BURST]` 마커 표시
- GUT: `test_ashen_rite_builds_stacks_and_emits_detonation`, `test_combo_summary_shows_burst_marker_when_ashen_rite_active`, `test_combo_effect_signal_spawns_projectile_and_focuses_event_camera`
...
| 조우/컷신/보스 페이즈 직결 연출 | combo/map progression 이후 직접 조우와 컷신, 보스 페이즈 설계 | 보류 (맵 전체 구현 이후 재개) |
