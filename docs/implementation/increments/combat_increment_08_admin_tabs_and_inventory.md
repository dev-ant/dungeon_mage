---
title: 전투 8차 작업 체크리스트 - 관리자 탭 구조와 장비 지급
doc_type: plan
status: active
...
이 문서는 [전투 우선 구현 계획](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/docs/implementation/plans/combat_first_build_plan.md)의 다음 대형 작업을 Claude가 바로 이어받을 수 있도록 잘게 쪼갠 체크리스트다.
...
- 이 에셋 기준은 더 이상 단순 handoff가 아니다. 2026-04-07 follow-up으로 [admin_menu.gd](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/scripts/admin/admin_menu.gd) 장비 탭에는 위 스킨을 직접 읽는 첫 그래픽 shell이 들어갔다.
...
- 같은 날짜 follow-up으로 [admin_menu.gd](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/scripts/admin/admin_menu.gd) 장비 탭에는 `Equipment.png` 상단 패널과 `Inventory.png` 우측 패널을 쓰는 그래픽 shell이 추가됐다. 이 shell은 `7슬롯 paperdoll 직접 선택 + 보유 인벤토리 5x4 viewport`를 제공하지만, 텍스트 fallback과 현재 `EQUIPMENT_PAGE_SIZE=5` page semantics도 함께 유지한다.
- 장비의 `최대 HP`, `최대 MP`, `MP 재생`, `기본 피해 감소`는 이미 실제 전투 수치에 연결되어 있다. 따라서 다음 단계는 수치 연결보다 `관리 흐름`과 `UI 구조` 쪽이 중심이다.
- 2026-04-07 기준 owner_core 쪽 preset source of truth는 이미 [game_state.gd](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/scripts/autoload/game_state.gd)에 잠겨 있다. hotbar preset은 `get_hotbar_preset_ids()`, `get_hotbar_preset_catalog()`, `get_next_hotbar_preset_id()`, `resolve_current_hotbar_preset_id()`, `get_hotbar_preset_state()`, `get_hotbar_preset_label()`, `get_hotbar_preset_data()`, `apply_hotbar_preset()`, `apply_next_hotbar_preset()`를 사용하고, equipment preset은 `get_equipment_preset_ids()`, `get_equipment_preset_catalog()`, `get_admin_equipment_preset_ids()`, `get_admin_equipment_preset_catalog()`, `get_next_admin_equipment_preset_id()`, `resolve_current_admin_equipment_preset_id()`, `get_admin_equipment_preset_state()`, `get_equipment_preset_label()`, `get_equipment_preset_data()`, `apply_equipment_preset()`, `apply_next_admin_equipment_preset()`를 사용한다.
- 2026-04-07 follow-up으로 [admin_menu.gd](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/scripts/admin/admin_menu.gd)의 hotbar preset 버튼/active state/next-preset cycle과 equipment preset cycle은 이제 위 `GameState` API를 직접 소비한다. local `PRESET_*`, `HOTBAR_PRESET_IDS`, `HOTBAR_PRESET_LABELS`, `["fire_burst", "ritual_control", "storm_tempo"]` source duplication은 이 문서 기준으로 정리됐다.
- `wind_tempest_drive`의 최종 canonical 구조는 아직 사용자 결정 대기 상태지만, admin hotbar preset 소비는 이미 `runtime-castable skill id` 기준으로 잠글 수 있으므로 이번 문서에서는 추가 질문 없이 통합 대상으로 본다.

## 2026-04-07 그래픽 GUI 구체화로 잠긴 기준

- 그래픽 장비 GUI의 첫 pass도 현재 텍스트 장비 탭이 가진 `후보(candidate) 탐색 -> 지급 -> 보유(owned) 확인 -> 장착` 의미를 유지한다. 즉, 그래픽 전환은 표현 교체이지 흐름 재설계가 아니다.
- 장비 paperdoll identity는 현재 `equipment_slot_order`와 같은 `weapon / offhand / head / body / legs / accessory_1 / accessory_2` 7슬롯을 그대로 쓴다. 슬롯 수를 늘리거나 합치지 않는다.
- 첫 그래픽 pass의 아이템 footprint 규칙은 `전 장비 1x1`로 잠근다. 현재 [equipment.json](/Users/leesanghyun/git-projects/java-projects/old/dungeon_mage/data/items/equipment.json)에는 점유 크기 메타데이터가 없으므로, multi-cell 장비는 데이터 스키마가 추가되기 전까지 구현 대상으로 올리지 않는다.
- drag/drop 점유 충돌 규칙은 HUD와 같은 `occupied target -> swap`을 따른다. 키보드 대체 경로는 현재 admin 장비 탭의 `T 포커스 전환 / N,R 순환 / Y,H 페이지 / B 정렬 / J 필터 / E 상호작용 / Q 지급`을 유지한다.
- 그래픽 장비 GUI가 들어와도 현재 텍스트 본문의 `Selection / View / Nav / List / Compare / Slot Stats / Panel Summary / Panel Flow` 정보 구조는 regression/debug fallback으로 남긴다.
- `Inventory.png` source image의 실제 visible inventory panel은 우측 패널 하나를 기준으로 `5열 x 4행`, `cell fill 14x14`, `cell pitch 16`, `panel-local origin (10,17)`로 잠근다. 이 좌표는 source image row/column scan으로 검증했고, first shell도 이 grid를 그대로 사용한다.
- `Equipment.png` source image의 상단 장비 패널은 `slot badge + paperdoll` container로 고정하고, 우측 helper area는 `4x4` 보조 grid rhythm을 가진다. 첫 shell은 여기서 `7슬롯 paperdoll direct select`만 먼저 노출한다.
- visible grid를 넘는 보유 장비 overflow는 `스크롤`이 아니라 `페이지 유지`로 잠근다. 이유는 현재 admin 장비 탭의 `Y/H` page navigation과 headless regression 구조가 이미 안정화되어 있기 때문이다.
- 첫 shell 구현은 기존 `EQUIPMENT_PAGE_SIZE=5` owned page semantics를 그대로 유지한다. 즉 `5x4 viewport`는 먼저 고정하고, 현재 페이지의 최대 5개 항목만 채우며 나머지 셀은 향후 multi-cell/drag GUI 확장을 위해 비워 둔다.
...
0. hotbar / equipment preset source를 `GameState` API 기준으로 먼저 전환한다.
...
### 0. preset source hardcode 제거

- `admin_menu.gd`는 hotbar preset 버튼과 preset cycle에서 local preset 상수를 직접 읽지 말고 `GameState.get_hotbar_preset_state()`와 `apply_next_hotbar_preset()`를 우선 사용한다.
- equipment preset cycle은 local `["fire_burst", "ritual_control", "storm_tempo"]` 배열을 직접 들지 말고 `GameState.get_admin_equipment_preset_state()`와 `apply_next_admin_equipment_preset()`를 우선 사용한다.
- equipment preset 버튼/요약이 확장되면 label 표시는 `GameState.get_equipment_preset_label()`을 기준으로 읽는다.
- 이 전환 이후 `admin_menu.gd` 내부에 preset row source of truth가 남아 있지 않아야 한다.
- preset payload는 이미 runtime-castable ID로 정규화되므로 GUI 소비자는 `wind_tempest_drive`의 예전 hybrid 여부나 현재 active canonical 여부를 별도 분기하지 않는다.
...
- 기존 관리자 기능이 깨지지 않는다.
- preset source of truth는 `admin_menu.gd` local 상수가 아니라 `GameState` preset API다.
- GUT와 헤드리스 검증이 모두 통과한다.
- 장비/인벤토리 UI가 최종적으로 그래픽 슬롯, 아이콘, 점유 크기, 위치 이동을 갖는 GUI로 확장 가능한 구조를 가진다.
- 위 그래픽 GUI 목표는 장착 panel shell과 inventory viewport 수준까지는 이미 구현됐지만, 아이콘 atlas, multi-cell 점유, drag/drop까지는 아직 완료가 아니다.
...
- 2026-04-07 기준 owner_core가 제공하는 preset API는 이미 준비 완료 상태다. friend_gui 증분은 UI 표현과 입력 처리만 책임지고, preset seed 정의를 새로 들고 있지 않도록 유지한다.
